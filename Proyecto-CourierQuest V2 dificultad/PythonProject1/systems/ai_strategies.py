...
class HardAI(CPUPlayer):
    ...
    def _calculate_optimal_path(self, target: Position):
        ...
        if path and len(path) > 1:
            self.current_path = path[1:]  # Excluir posición actual
            self.path_index = 0
            self.current_target = target
            self.stuck_counter = 0
            ...
            self.current_path = []
            self.current_target = target
            print(f"CPU {self.player_id}: A* falló, usando greedy")
            self._greedy_move_towards(target)

    def _follow_current_path(self):
        """Sigue el camino planificado."""
        ...
        if self.execute_move(next_pos, 0.016):
            ...
            if self.stuck_counter >= self.max_stuck:
                print(f"CPU {self.player_id}: Atascado, replanificando")
                self.current_path = []
                self.path_index = 0
                self.stuck_counter = 0
